...
<body>
    ...
    <div class="main">
        <form action="{{ route('all-books') }}" method="GET">
            <!-- Barre de recherche -->
            <input type="text" name="searchbox" id="searchbox" placeholder="Recherche"
                value="{{ old('searchbox', $request->get('searchbox')) }}">
            ...
            <div class="filter-options">
                <input type="radio" name="filter" id="title" class="filter" value="title" checked>
                        ...
                        <div class="book-details">
                            <h3>
                                @if (isset(Auth::user()->name))
                                    <a href="{{ route('details', ['idOuvrage' => $ouvrage->ouvrage_id]) }}"
                                        class="books-link">{{ $ouvrage->titre }}</a>
                                @else
                                    <a href="" class="books-link">{{ $ouvrage->titre }}</a>
                                @endif
                            </h3>
                            ...
                            <a class="pseudo"
                                href="{{ route('profil', ['idUser' => $ouvrage->fkUtilisateur->id]) }}">{{ $ouvrage->fkUtilisateur ? $ouvrage->fkUtilisateur->name : 'Pseudo non défini' }}</a>
                        </div>
                        <div class="MD">
                            @if (isset(Auth::user()->name) &&
                                    isset($ouvrage->fkUtilisateur->name) &&
                                    (Auth::user()->name === $ouvrage->fkUtilisateur->name || Auth::user()->estAdmin === 1))
                                <a href="/books/edit/{{ $ouvrage->ouvrage_id }}">Modifier</a> |
                                <a onclick="return confirmDelete('Êtes-vous sûr de vouloir supprimer l\'ouvrage ?');"
                                    href="{{ route('logic.deleteBook', ['idOuvrage' => $ouvrage->ouvrage_id]) }}">Supprimer</a>
                            @else
                            @endif
                            <div class="rate">
                                {{ $ouvrage->avg }}
